...
const ItemType = () => {
  ...
  const [editData, setEditData] = useState({ typeName: "", description: "", status: "" });
  ...
  const handleSearch = () => {
    let f = [...data];
    if (search) f = f.filter((i) => i.typeName === search);
  ...
  const handleEdit = (item) => {
    setEditId(item._id);
    setEditData({ typeName: item.typeName, description: item.description, status: item.status });
  };
      ...
      <div className="search-container">
        ...
        <div className="table-container">
          <table>
            <thead>
              <tr>
                <th>S No</th><th>Type Name</th><th>Description</th><th>Status</th><th>Action</th>
              </tr>
            </thead>
            <tbody>
              {filtered.length > 0 ? filtered.map((item, i) => (
                <tr key={item._id}>
                  ...
                  <td>
                    {editId === item._id ? (
                      <select value={editData.typeName}
                        onChange={(e) => setEditData({ ...editData, typeName: e.target.value })}>
                        {TYPE_OPTIONS.map((t) => <option key={t}>{t}</option>)}
                      </select>
                    ) : item.typeName}
                  </td>
