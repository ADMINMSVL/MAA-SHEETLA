...
/* ── Item-name typeahead (table cell) ── */
const ItemTA = ({ value, suggestions, onSelect, onChange }) => {
  ...
  return (
    <div style={{ position: "relative" }} ref={ref}>
      <input className="cgin-item-input cgin-item-wide" type="text" value={value}
        autoComplete="off" placeholder="Search item…"
        onChange={(e) => { onChange(e.target.value); setShow(true); }}
        onFocus={() => setShow(true)} />
      {show && value && filtered.length > 0 && (
        <ul className="po-suggestion-list" style={{ top: 30, zIndex: 999 }}>
          {filtered.slice(0, 10).map((s) => <li key={s} onMouseDown={() => { onSelect(s); setShow(false); }}>{s}</li>)}
        </ul>
      )}
    </div>
  );
...
const CreatePurchaseOrder = () => {
  ...
  useEffect(() => {
    ...
    const go = async () => {
      try {
        ...
        const matching = allSeq.filter((r) =>
        ...
        const last = matching.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))[0];
        let poNo = "PO01";
        if (last) {
          const nextInc   = Math.max(...matching.map((r) => Number(r.incrementNo))) + 1;
          ...
          const prefix    = last.entityPrefix || "PO";
          let datePart    = "";
          if (useDate) {
            const d = new Date();
          ...
          poNo = `${prefix}${datePart}${String(nextInc).padStart(digits,"0")}`;
        }
  ...
  const handleRowChange = (idx, field, value) => {
    setRows((prev) => {
      ...
      if (field === "itemName") {
        ...
        if (it) { next[idx].itemCode = it.itemCode || ""; next[idx].uom = it.uom || ""; next[idx].itemCategory = it.category || next[idx].itemCategory; }
      }
      return next;
    });
  };
  ...
  const itemsForRow = (rowIdx) => { const cat=rows[rowIdx].itemCategory; const list=cat?allItems.filter((i)=>i.category===cat):allItems; return [...new Set(list.map((i)=>i.itemName).filter(Boolean))].sort(); };
  ...
  const handleSave = async () => {
    ...
    try {
      try {
        ...
        const poSeq = (seqSnap.data||[]).filter((r)=>(r.module&&r.module.toLowerCase().includes("purchase order"))||(r.businessEntity&&r.businessEntity.toLowerCase().includes("purchase order"))||(r.businessEntity&&r.businessEntity.toUpperCase()==="REQ.PO")).sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt))[0];
        if (poSeq) await axios.post(`${API_URL}/api/create-document-sequence`,{module:poSeq.module,businessEntity:poSeq.businessEntity,entityPrefix:poSeq.entityPrefix});
      } catch(seqErr){ console.warn("Seq skip:",seqErr.message); }
  ...
  const handleReset = () => {
    ...
    setActiveTab("Items");
    setForm((f)=>({...f,poType:"",partyCode:"",partyName:"",site:"",paymentMode:"",eta:"",dueDate:"",status:DEFAULT_STATUS,remarks:""}));
  };
    ...
    <div className="cpo-page">
      ...
      <div className="cgin-card cpo-tab-card">
        ...
        <div className="cpo-tab-bar">
          ...
          {/* grand total — always visible in tab bar */}
        ...
        {activeTab === "Items" && (
          <div className="cpo-tab-panel">
                    ...
                    <tr key={idx} className={row._checked ? "cgin-row-checked" : ""}>
                      ...
                      <td>
                        <select className="cgin-item-input cgin-item-wide" value={row.itemCategory}
                          onChange={(e)=>{ handleRowChange(idx,"itemCategory",e.target.value); handleRowChange(idx,"itemName",""); handleRowChange(idx,"itemCode",""); }}>
                          <option value="">- Select -</option>
                          {categoryNames.map((c)=><option key={c} value={c}>{c}</option>)}
                        </select>
                      </td>
                      <td><input className="cgin-item-input" value={row.itemCode} readOnly style={{background:"#f8fafc"}} /></td>
                      <td>
                        <ItemTA value={row.itemName} suggestions={itemsForRow(idx)}
                          onSelect={(v)=>handleRowChange(idx,"itemName",v)}
                          onChange={(v)=>handleRowChange(idx,"itemName",v)} />
                      </td>
                      <td><input className="cgin-item-input cgin-item-sm" value={row.uom} readOnly style={{background:"#f8fafc"}} /></td>
                      <td><input type="number" className="cgin-item-input cgin-item-num" value={row.qty} min="0" onChange={(e)=>handleRowChange(idx,"qty",e.target.value)} /></td>
                      <td><input type="number" className="cgin-item-input cgin-item-num" value={row.rate} min="0" onChange={(e)=>handleRowChange(idx,"rate",e.target.value)} /></td>
                      <td><input className="cgin-item-input cgin-item-num" value={row.basicAmount} readOnly style={{background:"#f8fafc",fontWeight:600}} /></td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
            ...
            <div className="cgin-insert-row-bar">
              ...
              <input type="number" className="cgin-insert-count" min={1} max={50} value={insertCount} onChange={(e)=>setInsertCount(e.target.value)} />
